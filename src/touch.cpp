...
MSP4021::MSP4021(SPIClass &spi, int pinCS_TOUCH, int pinCS_TFT, int pinDC_TFT, int screenWidth, int screenHeight, int pinRST_TFT, int pinMISO_TFT, int pinMISO_TOUCH, int pinMOSI, int pinCLK)
    : MSP4020(spi, pinCS_TFT, pinDC_TFT, screenWidth, screenHeight, pinRST_TFT, pinMISO_TFT, pinMISO_TOUCH, pinMOSI, pinCLK, pinCS_TOUCH) {
...
bool MSP4021::TReadRaw(
    ...
    x = this->_readRaw(0x90);
    y = this->_readRaw(0xD0);
    z1 = this->_readRaw(0xB0);
    z2 = this->_readRaw(0xC0);
    return true;
}
...
void MSP4021::_waitForTouch(uint16_t &x, uint16_t &y) {
    ...
    while (true) {
        ...
        bool stable = true;
        for (int i = 0; i < 5; i++) {
            if (abs((int)samplesX[i] - avgX) > stabilityThreshold || abs((int)samplesY[i] - avgY) > stabilityThreshold) {
                stable = false;
                break;
            }
        }
...
void MSP4021::_waitRelease() {
    constexpr uint8_t REQUIRED_RELEASE_SAMPLES = 3;
    uint8_t releaseSamples = 0;

    while (releaseSamples < REQUIRED_RELEASE_SAMPLES) {
        if (this->_isTouched())
            { releaseSamples = 0; }
        else
            { releaseSamples++; }
...
void MSP4021::_drawCalibrationPoint(int x, int y, uint16_t &rx, uint16_t &ry, bool circle) {
    const int size = 20;

    if (circle) { this->circleFill(x, y, 5, this->rgb(255, 0, 0));}
    else { this->rectFill(x, y, size, size, this->rgb(255, 0, 0)); }

    delay(80);
    this->_waitForTouch(rx, ry);
    this->_waitRelease();

    if (circle) { this->circleFill(x, y, 5, this->rgb(0, 255, 0)); }
    else { this->rectFill(x, y, size, size, this->rgb(0, 255, 0)); }

    delay(250);
    if (circle) { this->circleFill(x, y, 5, this->rgb(0, 0, 0)); }
    else {this->rectFill(x, y, size, size, this->rgb(0, 0, 0)); }
}
...
bool MSP4021::TCalibrate() {
    if (this->_PAUSED || !this->detected())
        { return false; }

    const bool previousSwapXY = this->_SWAP_XY;
    const bool previousInvertX = this->_INVERT_X;
    const bool previousInvertY = this->_INVERT_Y;
    ...
    for (int i = 0; i < count; i++) {
        uint16_t tx, ty;
        this->_drawCalibrationPoint(sx[i], sy[i], tx, ty, true);
        this->_applyOrientation(tx, ty);
        rx[i] = (float)tx;
        ry[i] = (float)ty;
    }
    if (this->_affineCalibration(sx, sy, rx, ry, count)) {
        this->_CALIBRATED = true;
        return true;
    }

    this->_SWAP_XY = previousSwapXY;
    this->_INVERT_X = previousInvertX;
    this->_INVERT_Y = previousInvertY;
    return false;
}
