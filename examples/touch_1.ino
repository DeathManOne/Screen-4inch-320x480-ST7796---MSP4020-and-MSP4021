/*
 * examples/touch_1.ino
 *
 * Copyright (c) 2026 DeathManOne
 * https://github.com/DeathManOne
 *
 * This file is part of the ST7796S library.
 *
 * Licensed under the GNU GPL v3 or later.
 * See LICENSE file for details.
 */

#include <Arduino.h>
#include <SPI.h>
#include <MSP4021.h>

#define TFT_CS 10
#define TFT_DC 5
#define TFT_RST -1 // 3.3v or an available pin you want

#define TOUCH_CS 4
#define TFT_MISO A3
#define TOUCH_MISO 12
#define TFT_MOSI 11
#define TFT_CLK 13

#define SCREEN_WIDTH 320
#define SCREEN_HEIGHT 480

ST7796S::MSP4021 *_TFT;

void setup() {
  SPI.begin(TFT_CLK, TOUCH_MISO, TFT_MOSI);

  _TFT = new ST7796S::MSP4021(
    SPI,
    TOUCH_CS,
    TFT_CS,
    TFT_DC,
    SCREEN_WIDTH,
    SCREEN_HEIGHT,
    TFT_RST,
    TFT_MISO,
    TOUCH_MISO,
    TFT_MOSI,
    TFT_CLK
  );
  _TFT->setRotation(2);
  while (!_TFT->TCalibrate());
  _TFT->fillScreen(_TFT->rgb(0, 0, 0));
}

void loop() {
  delay(500);

  int x=0, y=0;
  bool antiSmoothing = true; // optional | default = true
  if (_TFT->TRead(x, y, antiSmoothing)) {
    _TFT->rectFill(x, y, 20, 20, _TFT->rgb(0, 255, 0));
    delay(1500);
    _TFT->rectFill(x, y, 20, 20, _TFT->rgb(0, 0, 0));
  }
}
